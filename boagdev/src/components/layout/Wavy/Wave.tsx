...
const Wave: React.FC<WaveProps> = ({ 
  ...
  const drawWave = useCallback(() => {
    ...
    const numPoints = Math.min(canvas.width * 2, 3000);
    ...
    for (let x = 0; x < canvas.width; x += stepSize) {
      ...
      const ampModMain = modMainActive 
        ? 10 * (Math.sin(ampModOne) + Math.sin(ampModTwo) + Math.random() * Math.sin(ampModThree))
        : 0;
  ...
  const updateWave = useCallback(() => {
    ...
    // phase
    setPhase(prev => 
      Math.random() < PHASE.DECREMENT_THRESHOLD
        ? prev - PHASE.DECREMENT
        : prev + PHASE.INCREMENT
    );
  }, [amplitude, frequency]);
